...
from config_param import (
    ...
    # ---------------- NEW: q-layer architecture + modulation params ----------------
    Q_LAYER_ARCH,
    Q_MOD_DELTA,
    Q_MOD_MU,
    Q_MOD_NU,
...
class AgentProtocol(IProtocol):
    ...
    def initialize(self):
        ...
        arch = str(Q_LAYER_ARCH).strip().upper()
        if arch not in ("FORCE_U", "MODULATE_KE_TAU"):
            arch = "FORCE_U"
        self.q_layer_arch = arch
        ...
        # Last commanded velocity BEFORE clamping (world coordinates).
        self.desired_velocity_raw: Tuple[float, float, float] = (0.0, 0.0, 0.0)

        # Anti-windup per-tick flag (set in CONTROL loop, read in SOLITON loop)
        self._aw_active: bool = False

        # ---------------- NEW: q -> gain modulation internal state ----------------
        self.k_e_tau_base: float = float(K_E_TAU) if math.isfinite(float(K_E_TAU)) else 0.0
        self.k_e_tau_eff: float = float(self.k_e_tau_base)
        self.q0: float = max(float(Q_MOD_EPS), 1e-9)
        self.q_mod: float = 0.0
        self.q_mod_f: float = 0.0
    ...
    # ---------------- NEW: q -> K_E_TAU modulator ----------------
    def _update_ke_tau_modulator(self, *, abs_q: float, freeze: bool) -> None:
        """
        Update K_E_TAU_eff = K_E_TAU * (1 + Q_MOD_DELTA * m_f), where m_f in [0,1].
        Only increases gain (never decreases below base).
        """
        base = float(self.k_e_tau_base)
        if not (math.isfinite(base) and base >= 0.0):
            base = 0.0
        self.k_e_tau_base = base

        # If not in modulate mode, keep base.
        if not (self.use_q_layer and self.q_layer_arch == "MODULATE_KE_TAU"):
            self.k_e_tau_eff = base
            self.q_mod = 0.0
            self.q_mod_f = 0.0
            return

        if freeze:
            # Optionally freeze/zero on saturation to avoid runaway.
            if bool(Q_MOD_FREEZE_ON_SAT):
                self.q_mod = 0.0
                self.q_mod_f = 0.0
                self.k_e_tau_eff = base
            return
        ...
        abs_q = float(abs_q)
        if not math.isfinite(abs_q):
            abs_q = 0.0

        mu = float(Q_MOD_MU)
        if not (math.isfinite(mu) and 0.0 <= mu <= 1.0):
            mu = 0.05

        nu = float(Q_MOD_NU)
        if not (math.isfinite(nu) and 0.0 <= nu <= 1.0):
            nu = 0.05
        ...
        # Robust scale tracker for normalization:
        # - keep q0 >= abs_q (prevents q0 being tiny and giving huge z)
        # - EMA to smooth
        q0 = float(self.q0)
        if not (math.isfinite(q0) and q0 > 0.0):
            q0 = eps
        q0 = max(q0, abs_q, eps)
        q0 = (1.0 - mu) * q0 + mu * abs_q
        q0 = max(q0, eps)
        self.q0 = float(q0)

        # Normalized magnitude
        z = abs_q / q0 if q0 > 0.0 else 0.0
        if not math.isfinite(z):
            z = 0.0
        z = max(0.0, z)

        # Map to [0,1]
        if bool(Q_MOD_USE_TANH):
            gain = float(Q_MOD_TANH_GAIN)
        ...
        # Low-pass filter
        m_f = float(self.q_mod_f)
        if not math.isfinite(m_f):
            m_f = 0.0
        m_f = (1.0 - nu) * m_f + nu * m
        m_f = max(0.0, min(1.0, m_f))

        self.q_mod = float(m)
        self.q_mod_f = float(m_f)

        delta = float(Q_MOD_DELTA)
        if not (math.isfinite(delta) and delta >= 0.0):
            delta = 0.0

        self.k_e_tau_eff = float(base * (1.0 + delta * m_f))
    ...
    def update_error_soliton_q(
        ...
        *,
        ...
        """
        Discrete-time error-soliton update for q.

        Variants selected by KDV_TYPE (strings):
        - "ORIGINAL": central form (your old): dq_adv = -a * q * (q_succ - q_pred)
        - "BURGERS" : stable conservative-flux form (Rusanov) + CFL substepping
                        (name kept for compatibility; it's "RUSANOV flux" in practice)

        CONSISTENCY NOTE:
        q_ss_out is ALWAYS computed at UPDATED q_next for both modes.
        """
        ...
        # Parameters (from config_param)
        a = float(K_U_STEEPEN) if math.isfinite(float(K_U_STEEPEN)) else 0.0
        kappa = float(KAPPA_U_DISP) if math.isfinite(float(KAPPA_U_DISP)) else 0.0
        beta = float(BETA_Q) if math.isfinite(float(BETA_Q)) else 0.0
        gamma = float(GAMMA_Q_CUBIC) if math.isfinite(float(GAMMA_Q_CUBIC)) else 0.0
        k_force = float(K_E_TAU) if math.isfinite(float(K_E_TAU)) else 0.0

        # Pick variant (must be "ORIGINAL" or "BURGERS")
        try:
            kdvt = str(KDV_TYPE).strip().upper()
        except Exception:
            kdvt = "BURGERS"
        if kdvt not in ("ORIGINAL", "BURGERS"):
            kdvt = "BURGERS"

        # Shared stencil helpers (debug continuity)
        q_s = float(q_succ - q_pred)        # central ~ q_x
        q_sss = float(q_ss_succ - q_ss_pred)  # ~ q_xxx (via neighbors' q_ss)

        # ---------------------------
        # Variant A: ORIGINAL (old)
        # ---------------------------
        if kdvt == "ORIGINAL":
            dq_steepen = float(-a * q * q_s)
            dq_disp = float(kappa * q_sss)
            dq_damp = float(-beta * q)

            # cubic damping computed ONLY if gamma != 0 to avoid overflow when gamma==0
            dq_damp_cubic = 0.0
            if gamma != 0.0:
                # guard: cube root of ~1e308 is ~5.6e102
                if abs(q) > 5.0e102:
                    dq_damp_cubic = float(-gamma * math.copysign(1.0, q) * 1.0e308)
                else:
                    dq_damp_cubic = float(-gamma * (q * q * q))

            dq_force = float(k_force * e_tau_f)

            dq = float(dq_steepen + dq_disp + dq_damp + dq_damp_cubic + dq_force)
            q_next = float(q + dt * dq) if dt > 0.0 else float(q)

            # CONSISTENT: curvature at UPDATED q_next
            ...
            terms_debug = {
                ...
                "q_s": q_s,
                "q_sss": q_sss,
                "dq_steepen": dq_steepen,
                "dq_adv": dq_steepen,   # bridge key
                "dq_diff": 0.0,
                "dq_disp": dq_disp,
                "dq_damp": dq_damp,
            ...
            # also keep q_ss in debug consistent with output
            terms_debug["q_ss"] = float(q_ss_out)

            if not (math.isfinite(q_next) and math.isfinite(dq)):
                return 0.0, 0.0, 0.0, {k: 0.0 for k in terms_debug}

            return float(q_next), float(q_ss_out), float(dq), terms_debug

        # -----------------------------------------
        # Variant B: "BURGERS" (Rusanov flux + CFL)
        # -----------------------------------------

        # Optional extra diffusion (keep 0.0 to match your current design)
        nu_q = 0.0

        def _rhs(q_local: float) -> Tuple[float, dict]:
            ql = float(q_local)
            if not math.isfinite(ql):
                ql = 0.0

            # curvature at ql
            q_ss_local = float(q_succ - 2.0 * ql + q_pred)
            q_sss_local = float(q_ss_succ - q_ss_pred)
            ...
            dq_disp = float(kappa * q_sss_local)
            dq_damp = float(-beta * ql)

            # cubic damping safely ONLY if gamma != 0
            if gamma != 0.0:
                if abs(ql) > 5.0e102:
        ...
        if dt <= 0.0:
            dq0, terms0 = _rhs(q)
            q_next0 = float(q)
            q_ss_out0 = float(q_succ - 2.0 * q_next0 + q_pred)
            if not math.isfinite(q_ss_out0):
                q_ss_out0 = 0.0
            if not math.isfinite(dq0):
                dq0 = 0.0

            terms0["dq_steepen"] = float(terms0.get("dq_adv", 0.0))
            terms0["q_s"] = float(q_succ - q_pred)
            terms0["q_ss"] = float(q_ss_out0)  # consistent

            return float(q_next0), float(q_ss_out0), float(dq0), terms0
        ...
        for _ in range(n_sub):
            ...
            if not math.isfinite(dq_last):
                dq_last = 0.0
            q_next_sub = float(q_curr + dt_sub * dq_last)
            if not math.isfinite(q_next_sub):
                q_next_sub = 0.0
            q_curr = q_next_sub
        ...
        terms_debug["q_ss"] = float(q_ss_out)  # enforce consistency with output

        if not (math.isfinite(q_next) and math.isfinite(dq_out)):
            return 0.0, 0.0, 0.0, {k: 0.0 for k in terms_debug}

        return float(q_next), float(q_ss_out), float(dq_out), terms_debug
    ...
    def update_control_u(
        ...
        *,
        ...
        du_diff = float(KAPPA_U_DIFF * u_ss)

        du_from_q = float(K_Q_TO_U * q_f)

        # allow override for modulated K_E_TAU
        k_e = float(K_E_TAU)
        if k_e_tau_override is not None:
            k_e = self._safe_float(k_e_tau_override, default=float(K_E_TAU))

        du_from_e_tau = 0.0
        if e_tau is not None:
            du_from_e_tau = float(k_e * self._safe_float(e_tau))
    ...
    def handle_timer(self, timer: str):
        ...
        if timer == CONTROL_LOOP_TIMER_STR:
            ...
            if self.target_state is not None:
                ...
                # Compute e_tau ALWAYS (needed for MODULATE_KE_TAU mode, and helpful for telemetry)
                e_tau, e_tau_eff, e_tau_used_raw = self.compute_e_tau_used(
                    pred_gap=pred_gap,
                    succ_gap=succ_gap,
                    t_hat=t_hat,
                    r_eff=r_eff,
                ...
                # Update modulator (uses previous tick saturation flag as "freeze" input)
                abs_q = abs(float(self.q)) if math.isfinite(float(self.q)) else 0.0
                self._update_ke_tau_modulator(abs_q=abs_q, freeze=bool(getattr(self, "_aw_active", False)))

                # Select architecture behavior for u-loop forcing terms
                # - FORCE_U: q injected, no direct e_tau in u (legacy behavior)
                # - MODULATE_KE_TAU: no q->u, direct e_tau in u with modulated K_E_TAU_eff
                use_force_u = bool(self.use_q_layer and self.q_layer_arch == "FORCE_U")
                use_mod_gain = bool(self.use_q_layer and self.q_layer_arch == "MODULATE_KE_TAU")

                q_for_u = 0.0
                e_tau_for_u: Optional[float] = None
                k_e_override: Optional[float] = None

                if not self.use_q_layer:
                    # no q-layer: direct e_tau with base K_E_TAU
                    q_for_u = 0.0
                    e_tau_for_u = float(e_tau_used_raw)
                    k_e_override = None
                else:
                    if use_force_u:
                        ...
                        q_for_u = 0.0
                        e_tau_for_u = float(e_tau_used_raw)
                        k_e_override = float(self.k_e_tau_eff)
                    else:
                        # fallback safe
                        q_for_u = float(self.q)
                        e_tau_for_u = None
                ...
                # --- First pass: compute u candidate with selected architecture forcing ---
                u_prev = float(self.u)
                ...
                # Store desired velocity before clamping
                self.desired_velocity_raw = v_cmd_candidate
                ...
                # If saturated, optionally freeze modulator for next tick (avoid runaway)
                if self._aw_active and bool(Q_MOD_FREEZE_ON_SAT):
                    self._update_ke_tau_modulator(abs_q=abs_q, freeze=True)
                ...
                if SIM_DEBUG:
                    ...
                    print(
                        f"Agent {self.node_id} tangential: {aw_str}, arch={self.q_layer_arch}, "
                        f"e_tau={self.last_e_tau:.3f}, e_tau_eff={self.last_e_tau_eff:.3f}, "
                        f"u_pred={u_pred:.3f}, u_succ={u_succ:.3f}, u={self.u:.3f}, "
                        f"q={self.q:.3f}, u_s={u_s_dbg:.3f}, delta_u={self.delta_u:.3f}, "
                        f"K_E_TAU_eff={self.k_e_tau_eff:.3f}, v_tau={v_tau}"
                    )
            ...
            if self.velocity_handler is not None:
                ...
                # Clamp to mobility limits and apply.
    ...
    def handle_telemetry(self, telemetry: Telemetry) -> None:
        ...
        # Use the last stored commands (post-clamp and pre-clamp) to compute effort/saturation metrics.
        v_cmd = getattr(self, "desired_velocity", (0.0, 0.0, 0.0))
        v_cmd_raw = getattr(self, "desired_velocity_raw", (0.0, 0.0, 0.0))

        v_xy = float(math.hypot(float(v_cmd[0]), float(v_cmd[1])))
        v_raw_xy = float(math.hypot(float(v_cmd_raw[0]), float(v_cmd_raw[1])))

        if not math.isfinite(v_xy):
            v_xy = 0.0
        if not math.isfinite(v_raw_xy):
            v_raw_xy = 0.0
        ...
        self._telemetry_rows.append(
            {
                ...
                # Tangential spacing error (signed). In the metrics we use e(t)=|e_tau(t)|.
                ...
                # Commanded speed norms in XY:
                # - v_cmd_raw_xy: before clamping to VM_MAX_SPEED_XY
                # - v_cmd_xy: after clamping (effective command sent to the mobility model)
                "v_cmd_raw_xy": float(v_raw_xy),
                "v_cmd_xy": float(v_xy),
            }
        )
