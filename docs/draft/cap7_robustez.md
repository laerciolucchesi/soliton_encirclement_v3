...
- [x] **Churn / falhas concorrentes — FEITO (ver §7.2.4).** Achado: eventos discretos = graciosos;
      churn contínuo = vantagem evapora + dano ocasional (regime mismatch, clipe insuficiente) →
      **gate** torna seguro. Os "3 fixes" (líquido-zero/carimbo-N/fallback) reclassificados: não são
      bugs que quebram o enxame (baseline cobre); o gate resolve a segurança.
- [x] **Track C — alvo MÓVEL — FEITO (ver §7.2.5).** Movimento modula o benefício (pleno em
      cruzeiro, diluído em manobra), sem modo de falha novo, tracking nunca degradado. Fase 3 ✅.
